...
- **多帳號** — 支援多個帳號，每個帳號獨立儲存 Moodle 網址；切換帳號即切換學校
...
管理多帳號。每個帳號獨立儲存帳密、token 和 **Moodle 網址**，切換帳號即可切換學校平台。

```bash
e3cli profile                  # 列出所有帳號（含學校網址）
e3cli profile use <名稱>       # 切換到指定帳號（同時切換 Moodle 網址）
e3cli profile remove <名稱>    # 刪除帳號
```

切換帳號時，`config.toml` 中的 Moodle 網址會自動更新為該帳號所屬學校。
...
# 切換學校
e3cli profile use nycu    # → 連線 NYCU E3
e3cli profile use ntu     # → 連線 NTU COOL
```
...
| `~/.e3cli/token` | 當前帳號的 Moodle API token (chmod 600) |
| `~/.e3cli/active_profile` | 當前帳號名稱 |
| `~/.e3cli/profiles/<名稱>/` | 各帳號的帳密、token、金鑰、Moodle 網址 |
